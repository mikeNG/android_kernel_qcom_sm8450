...
#ifdef FTS_NOUSE_FILE_READ_WRITE
static char csv_res_buf[CSV_BUFFER_LEN] ={0};
static char txt_res_buf[TXT_BUFFER_LEN] ={0};
int csv_use_buf_size = 0;
int txt_use_buf_size = 0;
#endif
...
{
#ifdef FTS_NOUSE_FILE_READ_WRITE
//save data at buf,no support fail result
    FTS_TEST_FUNC_ENTER();

    if(strncmp(file_name, FTS_CSV_FILE_NAME, sizeof(FTS_CSV_FILE_NAME)-1) == 0)
    {
        FTS_INFO("file name %s,now save at buf", file_name);
		memset(csv_res_buf,0,sizeof(csv_res_buf));
		memcpy(csv_res_buf,data_buf,len);
		csv_use_buf_size = len;
	}
	else if(strncmp(file_name, FTS_TXT_FILE_NAME, sizeof(FTS_TXT_FILE_NAME)-1) == 0)
	{
        FTS_INFO("file name %s,now save at buf", file_name);
		memset(txt_res_buf,0,sizeof(txt_res_buf));
		memcpy(txt_res_buf,data_buf,len);
		txt_use_buf_size = len;
	}
    else
    {
        FTS_TEST_ERROR("error file name %s.",file_name);
		return -EINVAL;
	}
    ...
    if (NULL == pfile) {
        pfile = filp_open(filepath, O_TRUNC | O_CREAT | O_RDWR, 0444);
    }
    if (IS_ERR(pfile)) {
    ...
    struct fts_test *tdata, char *prefix, char *suffix, char *buf, int len)
{
    ...
    if (false == tdata->result) {
        snprintf(file_name, 128, "%s_%ld_%ld%s", prefix,
                 (long)tdata->tv.tv_sec, (long)tdata->tv.tv_nsec, suffix);
...
//#ifdef FTS_NOUSE_FILE_READ_WRITE
    ...
    if(loc+2048<txt_use_buf_size)
    {
    
	    mutex_lock(&input_dev->mutex);
	    size = snprintf(buf, 2048, "%s",&txt_res_buf[loc]);
	    loc = loc+2048;
	...
	{
	    mutex_lock(&input_dev->mutex);
	    read_size = txt_use_buf_size - loc;
		if(read_size>0)
		{  
		    size = snprintf(buf,read_size , "%s",&txt_res_buf[loc]);
		}
		loc = 0;
		memset(txt_res_buf,0,sizeof(txt_res_buf));
		txt_use_buf_size = 0;
...
//add by huanghongkun begin
#ifdef FTS_NOUSE_FILE_READ_WRITE
static struct proc_dir_entry *tp_test_proc_csv;
#define FTS_PROC_TP_DIFFER_CSV "fts_tp_test_csv"
static struct proc_dir_entry *tp_test_proc_txt;
#define FTS_PROC_TP_DIFFER_TXT "fts_tp_test_txt"
...
static int fts_test_proc_show_csv(struct seq_file *m, void *v)
{
	seq_printf(m, "%s", csv_res_buf);
	
	return 0;
}

static int fts_test_proc_show_txt(struct seq_file *m, void *v)
{
	seq_printf(m, "%s", txt_res_buf);
	
	return 0;
}


static int fts_test_proc_open_csv(struct inode *inode, struct file *file)
{
	return single_open(file, fts_test_proc_show_csv, NULL);
}

static int fts_test_proc_open_txt(struct inode *inode, struct file *file)
{
	return single_open(file, fts_test_proc_show_txt, NULL);
}


static const struct proc_ops tp_test_proc_fops_csv =
{
    .proc_open     = fts_test_proc_open_csv,
    .proc_read     = seq_read,
    .proc_write    = tp_test_proc_write,
};

static const struct proc_ops tp_test_proc_fops_txt =
{
    .proc_open     = fts_test_proc_open_txt,
    .proc_read     = seq_read,
    .proc_write    = tp_test_proc_write,
};
...
#ifdef FTS_TEST_USE_DEBUGFS
static struct dentry *fts_test_debugfs_dir;
static struct dentry *fts_test_debugfs_file;

static ssize_t fts_test_debugfs_read(struct file *file,
	char __user *user_buf, size_t len, loff_t *offset)
{
	...
	ret += snprintf(buf + ret, PAGE_SIZE, "FTS_INI_FILE_PATH:%s\n",
                     FTS_INI_FILE_PATH);
	ret += snprintf(buf + ret, PAGE_SIZE, "FTS_CSV_FILE_NAME:%s\n",
                     FTS_CSV_FILE_NAME);
	ret += snprintf(buf + ret, PAGE_SIZE, "FTS_TXT_FILE_NAME:%s\n",
                     FTS_TXT_FILE_NAME);

	return simple_read_from_buffer(user_buf, len, offset, buf, ret);
}

static ssize_t fts_test_debugfs_write(struct file *file,
	const char __user *user_buf,size_t len, loff_t *offset)
{
	...
	if (len > sizeof(fwname) - 1)
		return -EINVAL;

	if (copy_from_user(fwname, user_buf, len))
		return -EFAULT;
...
static const struct file_operations fts_test_debugfs_fops = {
	.owner = THIS_MODULE,
	.read = fts_test_debugfs_read,
	.write = fts_test_debugfs_write,
};
#endif
//add by huanghongkun end
...
//add by huanghongkun begin
#ifdef FTS_NOUSE_FILE_READ_WRITE	
	tp_test_proc_csv = proc_create(FTS_PROC_TP_DIFFER_CSV, 0444, NULL, &tp_test_proc_fops_csv);
	if (tp_test_proc_csv == NULL) {
		FTS_TEST_ERROR("[focal] %s() - ERROR: create tp_test_proc_csv proc()  failed.",  __func__);
	}

	tp_test_proc_txt = proc_create(FTS_PROC_TP_DIFFER_TXT, 0444, NULL, &tp_test_proc_fops_txt);
	if (tp_test_proc_txt == NULL) {
		FTS_TEST_ERROR("[focal] %s() - ERROR: create tp_test_proc_txt proc()  failed.",  __func__);
	}
#endif

#ifdef FTS_TEST_USE_DEBUGFS
        FTS_TEST_INFO("FTS test create debugfs file");

	fts_test_debugfs_dir = debugfs_create_dir("fts_capacity_test", NULL);
	if (!fts_test_debugfs_dir) {
		pr_err("%s: create dir fail\n", __func__);
	}

	fts_test_debugfs_file = debugfs_create_file("fts_test", 0666, fts_test_debugfs_dir, NULL, &fts_test_debugfs_fops);
	if (!fts_test_debugfs_file) {
		pr_err("%s: create fts_test_debugfs_file interface fail\n", __func__);
	}
#endif
//add by huanghongkun end
