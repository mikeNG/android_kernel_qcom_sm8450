			 ...
			 const struct i2c_device_id *id)
{
	...
	pr_info("%s: st21nfc_probe......success       \n",__func__);
	return 0;
...
{
	int status = -EINVAL;
	pr_info("%s: Loading st21nfc driver andrew (version %s)\n",
		__func__, DRIVER_VERSION);
	status = i2c_add_driver(&st21nfc_driver);
	pr_info("st21nfc_dev_init status = 0x%x\n", status);
	return status;
}
